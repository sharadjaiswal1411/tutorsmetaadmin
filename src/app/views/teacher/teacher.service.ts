...
{
  return this.http.get<any>(environment.API_URL+'teacher'+'/?search_text='+search_text,{headers});
}
 
if(status && !(search_text)&&!(category)&&!(subcategory))
{
  return this.http.get<any>(environment.API_URL+'teacher'+'/?status='+status,{headers});
}

if(subcategory &&(!status) && !(search_text)&&!(category))
{
  return this.http.get<any>(environment.API_URL+'teacher'+'/?subcategory='+subcategory,{headers});
}

if(category &&(!status) && !(search_text)&&!(subcategory))
{
  return this.http.get<any>(environment.API_URL+'teacher'+'/?category='+category,{headers});
}
...
return this.http.get<any>(environment.API_URL+'teacher'+'/?current_page='+current_page+'/?per_page='+per_page+'/?search_text='+search_text+'/?status'+status,{headers});
...
//         return this.http.get<any>(environment.API_URL+'subject'+'/?current_page='+current_page+'/?per_page='+per_page+'/?search_text='+search_text);
  ...
  viewSubjectData(id){
    ...
    return this.http.get<any>(environment.API_URL+'teacher/?search_text='+id,{headers});
  }

  deleteSubjectData(id){

    return this.http.delete<any>(environment.API_URL+'teacher/'+id,{headers});
  }

  getCurrentData(id){
    return this.http.get<any>(environment.API_URL+'teacher/'+id,{headers});
  }

  updateCurrentData(id,data){
    return this.http.put<any>(environment.API_URL+'teacher/'+id,data,{headers});
  }
}
