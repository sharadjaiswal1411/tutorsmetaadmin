...
export class EditComponent implements OnInit {
  ...
  constructor(private http: HttpClient, public router: Router,private activerouter:ActivatedRoute,private subjectdataService:CategoryService) { }

  ngOnInit(): void {
    let subjectdata=this.subjectdataService.getCurrentData(this.activerouter.snapshot.params.id).subscribe(data=>{
      console.log(data.result.data);
      this.subjectData=data.result.data.categoryDetails;
