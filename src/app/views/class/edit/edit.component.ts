...
export class EditComponent implements OnInit {
  ...
  constructor(private http: HttpClient, public router: Router,private activerouter:ActivatedRoute,private classService:ClassService) { }
  ...
  ngOnInit(): void {
    ...
    let subjectdata=this.classService.getCurrentData(this.activerouter.
      ...
      console.log(data.result.data.classDetails);
      this.subjectData=data.result.data.classDetails;
 var subcategoryId=data.result.data.classDetails.subcategoryId._id;
 var categoryId=data.result.data.classDetails.subcategoryId.categoryId._id;
...
submit(name:string,status:string,category:string,subcategory:string): Observable<any> {
  ...
  //console.log(headers);
    ...
    this.setSubject();
    return this.http.put<any>(environment.API_URL+'class/'+this.activerouter.snapshot.params.id,{name:name,status:status,subcategoryId:subcategory},{headers});
}
